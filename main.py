...
from src.regression import run_regression_cv, save_xy_plots_per_model
from src.classification import run_classification_cv, save_confusion_matrices_per_model
...
def main():
    ...
    cls_summary, cls_folds, cls_best, cls_best_by_model = run_classification_cv(
        Xc, yc, k_folds=K_FOLDS, random_state=RANDOM_STATE
    )
    save_confusion_matrices_per_model(
        cls_best_by_model,
        os.path.join(fig_dir, "confusion_matrices_by_model")
    )


    reg_summary, reg_folds, reg_best, reg_best_by_model = run_regression_cv(
        Xr, yr, k_folds=K_FOLDS, random_state=RANDOM_STATE
    )

    save_xy_plots_per_model(
        reg_best_by_model,
        os.path.join(fig_dir, "xy_plots_by_model"),
